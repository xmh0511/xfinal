...
#ifdef XFINAL_ENABLE_CLIENT_SSL
		...
		std::string to_body() const
		{
			...
			for (auto iter = form_data.begin(); iter != form_data.end(); ++iter)
			{
				...
				if (file_iter == file_form.end()) {
					...
					auto mime_type = xfinal::get_content_type(extension_name);
					content.append("Content-Type: ").append(std::string(mime_type.data(), mime_type.size())).append("\r\n");
					content.append("\r\n");
...
namespace xfinal {
    ...
    class ClientBase {
    public:
        using error_code = xfinal::error_code;
...
//                tmp<< save_content;
        ...
        public:
            client_response(const client_response& res):http_version(res.http_version),status_code(res.status_code),header(res.header),streambuf(0),content(streambuf),save_content(res.save_content)
            {
            }
        protected:
    ...
    protected:
        ...
        public:
            ...
            void set_timeout(long seconds = 0) noexcept {
				...
				auto& io = static_cast<asio::io_service&>(socket->get_executor().context());
                timer = std::unique_ptr<asio::steady_timer>(new asio::steady_timer(io));
    ...
    public:
        ...
        client_response request(const std::string &path,const multipart_form& multi_form) {
            ...
            return request<Method>(path,nonstd::string_view(body.data(),body.size()));
        }

        template<http_method Method>
        client_response request(const std::string &path = std::string("/"),
                                nonstd::string_view content = "") {
            auto type_name = http_method_to_name(Method);
        ...
        void request(const std::string &method, const std::string &path, nonstd::string_view content,std::function<void(const client_response&, const error_code &)> &&request_callback_) {
        ...
        /// Do not use concurrently with the synchronous request functions.
        template<http_method Method>
        void request(const std::string &path, nonstd::string_view content,std::function<void(const client_response&, const error_code &)> &&request_callback) {
            auto type_name = http_method_to_name(Method);
            std::string method(type_name.data(),type_name.size());
